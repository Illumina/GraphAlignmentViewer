...
def get_args():
    ...
    parser.add_argument(
        "--repeat_specs", dest='variant_catalog', type=str, help='Path to variant catalog json file used to run EH (default: "variant_catalog.json")', default='repeat-specs')
...
def main():
    ...
    repeat_graphs = populate_repeat_graphs(args.repeat_specs, args.file_format, reference_fasta=reference_fasta, flank_size=args.flank_size)
    ...
    for repeat_id in site_list:
        if args.node_grouping == 'NONE':
            node_grouping_list = []
        else if args.node_grouping = 'ALL':
