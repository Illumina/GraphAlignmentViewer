...
class ReferenceGraph(list):
    ...
    def __init__(self, repeat_spec, reference_fasta=None, flank_size=1000):
        ...
        if reference_fasta is None:
            self.append(ReferenceNode(node_id, 'N' * self.flank_size, False))
        else:
            self.append(ReferenceNode(node_id, reference_fasta.fetch(self.chrom, self.start - self.flank_size, self.start), False))
        node_id += 1
        if '(' not in self.repeat_unit:
            # Then entire string is a single loop
            self.append(ReferenceNode(1, self.repeat_unit, True, node_names[node_name_index]))
            node_id += 1
            node_name_index += 1
        else:
            repeat_unit_index = 0
            while repeat_unit_index < len(self.repeat_unit):
                ...
                if self.repeat_unit[repeat_unit_index] == '(':
                    self.append(ReferenceNode(node_id, str(self.repeat_unit[repeat_unit_index + 1:index2]), True,
                                              node_name=node_names[node_name_index]))
                    node_name_index += 1
                else:
                    self.append(ReferenceNode(node_id, str(self.repeat_unit[repeat_unit_index:index2]), False))
                node_id += 1
        ...
        if reference_fasta is None:
            self.append(ReferenceNode(node_id, 'N' * flank_size, False))
        else:
            self.append(ReferenceNode(node_id, reference_fasta.fetch(self.chrom, self.end, self.end + flank_size), False))
